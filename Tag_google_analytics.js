...
tagAnalyticsCNIL.CookieConsent = function () {
  ...
  // Par défaut nous considérons que le domaine est tout ce qu’il y a après « www »
	...
	// Effectue une demande de confirmation de DNT (Do Not Track) pour les utilisateurs d’IE
	function askDNTConfirmation() {
		var r = confirm('L’option « Do Not Track » de votre navigateur est activée, confirmez-vous l’activation de cette fonction de refus de pistage publicitaire&nbsp;?')
		return r;
	}
	...
	function isClickOnOptOut( evt) { // Si le nœud parent ou le nœud parent du parent est la bannière, on ignore le clic
		return(evt.target.parentNode.id == 'cookie-banner' || evt.target.parentNode.parentNode.id =='cookie-banner' || evt.target.id == 'optout-button')
	}

	function consent(evt) {
		if (!isClickOnOptOut(evt) ) { // On vérifie qu’il ne s’agit pas d’un clic sur la bannière
			if ( !clickprocessed) {
				evt.preventDefault();
	...
	// Cette fonction en cours de test permet de faire un appel GA afin de pouvoir compter le nombre de visites sans faire de suivi des utilisateurs
	// Cela crée un évènement « page » qui est consultable depuis le panneau evenement de GA
	// Cette méthode pourrait être utilisée pour comptabiliser les clics sur l’opt-out
	...
	// Tag Google Analytics, cette version est dite « Universal Analytics »
	...
	return {
		...
		start: function() {
			// Cette portion de code vérifie que le consentement n’a pas déjà été obtenu avant d’afficher la bannière
			var consentCookie =  getCookie('hasConsent');
			clickprocessed = false;
			if (!consentCookie) { // L’utilisateur n’a pas encore de cookie, on affiche la banniére et si il clique sur un autre élément que la bannière, on enregistre le consentement
